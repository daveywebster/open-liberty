 ...
 * <pre>
 * {@code
 * AppValidator.validateAppOn(SHARED_SERVER).withClass(MyInvalidClass.class).failsWith("CWWKZ0002E").run();
 * }
 * </pre>
 *
 * <p>
 * This creates a web application containing just {@code MyInvalidClass}, deploys it, checks that it fails to start and that a message containing {@code CWWKZ0002E} is found in the
...
public class AppValidator {
     ...
     * Usually the app should include only the classes needed to provoke the validation failure under test
     */
    public AppValidator withPackage(String packageName) {
        ...
        try {
            ...
            try {
                ...
                for (String packageName : packages) {
                    app = app.addPackage(packageName);
                }
                ...
                for (String stringToFind : stringsToFind) {
                    String logLine = server.waitForStringInLogUsingMark(stringToFind);
